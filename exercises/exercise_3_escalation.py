    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        analysis = llm.invoke([
            {"role": "system", "content": (
                ...
                "specific, context-rich questions that reference the relevant file, "
                "function, security concern, migration, test gap, or diff section."
            )},
...
def node_escalate(state: ReviewState) -> dict:
    ...
    questions = a.escalation_questions or [
        ...
        "Are there any migration, security, or rollout concerns reviewers should know?",
    ]
    answers = interrupt({
        "kind": "escalation",
        "pr_url": state["pr_url"],
        ...
        refined = llm.invoke([
            {
                "role": "system",
                "content": (
                    "You are refining a PR review after human escalation. Use the reviewer "
                    "answers as additional context, keep any unresolved blockers, and return "
                    "a complete structured PRAnalysis ready to post."
                ),
            },
            {
                "role": "user",
